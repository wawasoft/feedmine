...
struct CatalogCursor: Codable, Equatable, Sendable {
...
/// A bounded page returned by catalog browse/search operations.
///
/// A page may contain nodes, sources, or both depending on the query. The UI
/// never receives the complete catalog or a full subtree.
struct CatalogPage: Equatable, Sendable {
    let nodes: [CatalogNodeSummary]
...
struct CatalogNodePage: Equatable, Sendable {
...
struct TimelineCursor: Codable, Equatable, Sendable {
    let sortTimestamp: Date
    let itemID: String
}

/// A page of feed items for the content timeline.
struct TimelinePage: Equatable, Sendable {
    let items: [TimelineItemSummary]
...
enum SourceScope: Equatable, Sendable {
...
/// Shared page-size guardrail for the FeedEngine boundary.
enum FeedEnginePageLimit {
    static let defaultLimit = 50
    static let maximumLimit = 200

    static func bounded(_ requestedLimit: Int) -> Int {
        min(max(requestedLimit, 1), maximumLimit)
    }
}
