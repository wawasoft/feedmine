#!/usr/bin/env python3
"""Build a university list for every country in countries.json using Wikidata.

Queries the Wikidata SPARQL endpoint to find all universities (Q3918) per country,
extracting: name, official website, YouTube channel, and any RSS/podcast feeds.

Output: scripts/feed_discovery/data/universities/by_country/{slug}.json
         scripts/feed_discovery/data/universities/all_universities.json

Usage:
    python3 scripts/build_university_list.py                  # all countries
    python3 scripts/build_university_list.py --country brazil  # single country
    python3 scripts/build_university_list.py --fresh           # no cache
"""
...
# Wikidata Q-ID for "instance of" (P31) values we care about
UNIVERSITY_QID = "Q3918"       # university
COLLEGE_QID = "Q189004"        # college (often used interchangeably)
INSTITUTE_QID = "Q11427974"    # higher education institution (broader)

# SPARQL endpoint
SPARQL_ENDPOINT = "https://query.wikidata.org/sparql"

# Known country ISO2 -> Wikidata Q-ID map (built lazily + cached)
COUNTRY_QID_CACHE = Path(__file__).resolve().parent / "feed_discovery/data/universities/country_qids.json"

# University data output
DATA_DIR = Path(__file__).resolve().parent / "feed_discovery/data/universities"
...
def _get_country_qid(iso2: str, country_name: str) -> str | None:
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "FeedmineUniversityBot/1.0"})
...
def _fetch_universities_for_country(
    ...
    WHERE {{
      {{
        ?item wdt:P31 wd:{UNIVERSITY_QID}.
      }} UNION {{
        ?item wdt:P31 wd:{COLLEGE_QID}.
      }} UNION {{
        ?item wdt:P31 wd:{INSTITUTE_QID}.
...
def build_university_list(
    countries: dict | None = None,
    target_country: str | None = None,
    fresh: bool = False,
) -> dict[str, list[dict]]:
    """Main entry point: build university lists for all (or one) countries.

    Returns {country_slug: [university_dict, ...]}.
    """
    ...
    # Fetch universities per country
    ...
    for i, (slug, cdata) in enumerate(countries.items()):
        ...
        cache_path.write_text(json.dumps(unis, indent=2, ensure_ascii=False), encoding="utf-8")
        all_results[slug] = unis
...
def main():
    parser = argparse.ArgumentParser(description="Build university list from Wikidata")
    ...
    countries = _load_countries()
    build_university_list(countries, target_country=args.country, fresh=args.fresh)
