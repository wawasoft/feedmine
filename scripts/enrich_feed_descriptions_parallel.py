#!/usr/bin/env python3
"""Parallel version of enrich_feed_descriptions.py using ThreadPoolExecutor.

Speeds up enrichment ~20x by making concurrent DeepSeek API calls.
Same parquet format, same progress/resume, same signal handling.
The original sequential script code is imported as a module.

Usage:
    export DEEPSEEK_API_KEY=sk-...
    python3 scripts/enrich_feed_descriptions_parallel.py              # default 20 workers
    python3 scripts/enrich_feed_descriptions_parallel.py --workers 50 # aggressive
    python3 scripts/enrich_feed_descriptions_parallel.py --limit 100  # test
    python3 scripts/enrich_feed_descriptions_parallel.py --reset      # fresh start
"""

from __future__ import annotations

import json
import math
import os
import shutil
import signal
import sys
import time
import threading
from concurrent.futures import ThreadPoolExecutor, as_completed
from datetime import datetime, timezone
from pathlib import Path
from urllib.request import Request, urlopen
from urllib.error import URLError
from urllib.parse import urlparse

import pyarrow as pa
import pyarrow.parquet as pq

# ---------------------------------------------------------------------------
# Paths & constants
# ---------------------------------------------------------------------------

ROOT = Path(__file__).resolve().parent.parent
PARQUET_PATH = ROOT / "feeds_corpus_sources.parquet"
BACKUP_PATH = ROOT / "feeds_corpus_sources.backup.parquet"
PROGRESS_PATH = ROOT / "feeds_enrich_progress.json"
CONTENT_PARTS_DIR: Path | None = None

DEEPSEEK_MODEL = "deepseek-v4-flash"
DEEPSEEK_URL = "https://api.deepseek.com/v1/chat/completions"

MAX_TOKENS = 512
TEMPERATURE = 0.3
DELAY_BETWEEN_CALLS = 0.0  # No delay needed with concurrency
MAX_RETRIES = 4
RETRY_BASE_DELAY = 2.0
DEFAULT_WORKERS = 20

NEW_COLUMNS = ["ai_description", "ai_tags"]

# Thread safety for printing only
_print_lock = threading.Lock()


def safe_print(*args, **kwargs):
    with _print_lock:
        print(*args, **kwargs)


# ---------------------------------------------------------------------------
# Helpers (mostly same as original)
# ---------------------------------------------------------------------------

def load_progress() -> dict[str, dict]:
    if PROGRESS_PATH.exists():
        with open(PROGRESS_PATH) as f:
            return json.load(f)
    return {}


def save_progress(progress: dict[str, dict]) -> None:
    tmp = PROGRESS_PATH.with_suffix(".tmp")
    with open(tmp, "w") as f:
        json.dump(progress, f, ensure_ascii=False)
    tmp.replace(PROGRESS_PATH)


_signal_state = {"progress": {}, "df": None}


def _handle_interrupt(signum, frame):
    msg = f"\n⚠ Interrupted (signal {signum}) — saving progress..."
    print(msg, file=sys.stderr, flush=True)
    progress = _signal_state.get("progress", {})
    if progress:
        save_progress(progress)
        print(f"  Saved {len(progress)} entries", file=sys.stderr)
    sys.exit(0)


signal.signal(signal.SIGTERM, _handle_interrupt)
signal.signal(signal.SIGINT, _handle_interrupt)


def write_parquet(df) -> None:
    """Write parquet without holding a lock (called only from main thread).

    The lock was removed because:
    - write_parquet is only called from the main thread (never from workers)
    - copying the 5GB parquet while holding a lock blocks all workers
    - pyarrow's internal thread pool doesn't interact with Python locks
    """
    if PARQUET_PATH.exists():
        shutil.copy2(PARQUET_PATH, BACKUP_PATH)
    new_table = pa.Table.from_pandas(df)
    temporary = PARQUET_PATH.with_name(f".{PARQUET_PATH.name}.tmp")
    pq.write_table(new_table, temporary, compression="zstd")
    os.replace(temporary, PARQUET_PATH)


def safe_text(value: object) -> str:
    if value is None:
        return ""
    if isinstance(value, float) and math.isnan(value):
        return ""
    text = str(value).strip()
    return "" if text.casefold() in {"nan", "none", "<na>"} else text


def load_api_key() -> str | None:
    api_key = os.environ.get("DEEPSEEK_API_KEY")
    if api_key:
        return api_key
    env_path = ROOT / ".env"
    if env_path.exists():
        with open(env_path) as f:
            for line in f:
                line = line.strip()
                if line.startswith("DEEPSEEK_API_KEY="):
                    return line.split("=", 1)[1].strip().strip('"').strip("'")
    return None


def load_article_samples(parts_dir: Path | None, max_per_source: int = 5, max_chars: int = 700) -> dict[str, list[str]]:
    if parts_dir is None or not parts_dir.exists():
        return {}
    result: dict[str, list[str]] = {}
    columns = ["source_id", "title", "summary", "content_text", "position"]
    for path in sorted(parts_dir.glob("*.parquet")):
        parquet_file = pq.ParquetFile(path)
        for batch in parquet_file.iter_batches(columns=columns, batch_size=512):
            data = batch.to_pydict()
            for index, source_id in enumerate(data["source_id"]):
                source_id = str(source_id)
                samples = result.setdefault(source_id, [])
                if len(samples) >= max_per_source:
                    continue
                title = " ".join(str(data["title"][index] or "").split())
                body = " ".join(str(
                    data["summary"][index] or data["content_text"][index] or ""
                ).split())
                sample = " — ".join(part for part in (title, body) if part)[:max_chars]
                if sample and sample not in samples:
                    samples.append(sample)
    return result


def call_deepseek(feed: dict, api_key: str) -> dict | None:
    """Thread-safe DeepSeek API call (one feed → one API request)."""
    title = (feed.get("feed_title") or feed.get("source_title") or "").strip()
    desc = (feed.get("feed_description") or "").strip()
    site = (feed.get("site_url") or "").strip()

    parts = []
    if title:
        parts.append(f"Title: {title}")
    if desc:
        parts.append(f"Original description: {desc[:300]}")
    if site:
        try:
            domain = urlparse(site).netloc
            parts.append(f"Website: {domain}")
        except Exception:
            parts.append(f"Website: {site}")
    article_samples = feed.get("article_samples") or []
    if article_samples:
        parts.append("Recent article samples derived from fetched feed entries:")
        parts.extend(f"- {sample}" for sample in article_samples)

    feed_text = "\n".join(parts)

    system_prompt = (
        "You are a multilingual RSS/Atom feed curator. "
        "Given feed metadata and, when available, samples from its fetched entries, "
        "write a SHORT description "
        "(1-2 sentences) in English explaining what the feed covers, and assign "
        "3-5 relevant tags in English. "
        "Infer the real recurring subject primarily from the entry samples, not from "
        "an ambiguous feed name. If the feed is in another language, convey its topic "
        "accurately in English. "
        "Respond ONLY with a JSON object:\n"
        '{"description": "...", "tags": "tag1, tag2, tag3"}\n'
        "No markdown, no explanation — just the JSON."
    )

    user_prompt = (
        "Analyze this RSS/Atom feed and produce a short description and tags in English:\n\n"
        f"{feed_text}"
    )

    for attempt in range(MAX_RETRIES):
        try:
            body = json.dumps({
                "model": DEEPSEEK_MODEL,
                "messages": [
                    {"role": "system", "content": system_prompt},
                    {"role": "user", "content": user_prompt},
                ],
                "temperature": TEMPERATURE,
                "max_tokens": MAX_TOKENS,
                "response_format": {"type": "json_object"},
            }).encode()

            req = Request(
                DEEPSEEK_URL,
                data=body,
                headers={
                    "Content-Type": "application/json",
                    "Authorization": f"Bearer {api_key}",
                },
            )
            resp = urlopen(req, timeout=60)
            resp_body = json.loads(resp.read().decode())
            content = resp_body["choices"][0]["message"]["content"]
            data = json.loads(content)

            return {
                "ai_description": str(data.get("description", "")).strip(),
                "ai_tags": str(data.get("tags", "")).strip(),
            }

        except (json.JSONDecodeError, KeyError) as e:
            if attempt < MAX_RETRIES - 1:
                time.sleep(RETRY_BASE_DELAY * (attempt + 1))
        except URLError as e:
            if attempt < MAX_RETRIES - 1:
                delay = RETRY_BASE_DELAY * (attempt + 1)
                time.sleep(delay)
        except Exception as e:
            if attempt < MAX_RETRIES - 1:
                time.sleep(RETRY_BASE_DELAY * (attempt + 1))

    return None


# ---------------------------------------------------------------------------
# Parallel enrichment core
# ---------------------------------------------------------------------------

def enrich_parquet_parallel(
    limit: int = 0,
    reset: bool = False,
    dry_run: bool = False,
    parts_dir: Path | None = None,
    workers: int = DEFAULT_WORKERS,
) -> None:
    """Main enrichment — parallel API calls via ThreadPoolExecutor."""
    if not PARQUET_PATH.exists():
        print(f"ERROR: {PARQUET_PATH} not found")
        sys.exit(1)

    print(f"Reading {PARQUET_PATH}...")
    table = pq.read_table(PARQUET_PATH)
    df = table.to_pandas()
    total = len(df)
    print(f"  {total} total feeds")

    # Add new columns if missing
    for col in NEW_COLUMNS:
        if col not in df.columns:
            df[col] = ""

    # Progress tracking
    progress = {} if reset else load_progress()
    if reset:
        print("  --reset: starting fresh")
        for col in NEW_COLUMNS:
            if col in df.columns:
                df[col] = ""

    done_mask = df["status"] == "done"
    done_count = done_mask.sum()
    print(f"  {done_count} feeds with status=done")

    article_samples = load_article_samples(parts_dir)
    if parts_dir is not None:
        print(f"  Content samples for {len(article_samples)} feeds from {parts_dir}")

    # Find feeds needing enrichment
    needs_enrichment = []
    skipped_progress = 0
    skipped_parquet = 0
    for idx in df.index:
        if not done_mask[idx]:
            continue
        sid = df.at[idx, "source_id"]
        if isinstance(sid, float):
            sid = str(sid)
        elif hasattr(sid, "item"):
            sid = sid.item()
        sid = str(sid)

        if sid in progress and progress[sid].get("ai_description", "").strip():
            skipped_progress += 1
            df.at[idx, "ai_description"] = progress[sid]["ai_description"]
            df.at[idx, "ai_tags"] = progress[sid].get("ai_tags", "")
            continue
        existing_desc = df.at[idx, "ai_description"] if "ai_description" in df.columns else ""
        if isinstance(existing_desc, str) and existing_desc.strip():
            skipped_parquet += 1
            existing_tags = df.at[idx, "ai_tags"] if "ai_tags" in df.columns else ""
            progress[sid] = {
                "ai_description": existing_desc,
                "ai_tags": str(existing_tags) if existing_tags and not isinstance(existing_tags, float) else "",
            }
            continue
        needs_enrichment.append(idx)

    print(f"  Skipped: {skipped_progress} from progress, {skipped_parquet} from parquet")
    print(f"  {len(needs_enrichment)} feeds need enrichment")

    if limit and limit > 0:
        needs_enrichment = needs_enrichment[:limit]
        print(f"  --limit {limit}: processing {len(needs_enrichment)}")

    if not needs_enrichment:
        print("  Nothing to do!")
        return

    if dry_run:
        est_per_feed = 1.5  # seconds per API call
        est_min = len(needs_enrichment) * est_per_feed / workers / 60
        print(f"\n  DRY-RUN: would process {len(needs_enrichment)} feeds "
              f"with {workers} workers (~{est_min:.0f} min)")
        return

    api_key = load_api_key()
    if not api_key:
        print("ERROR: DEEPSEEK_API_KEY not set")
        sys.exit(1)

    _signal_state["progress"] = progress
    _signal_state["df"] = df

    FLUSH_EVERY = 500  # flush every N completed feeds (was 200)
    total_needed = len(needs_enrichment)
    est_min = total_needed * 1.5 / workers / 60
    print(f"\n  Processing {total_needed} feeds with {workers} workers (~{est_min:.0f} min)")
    print(f"  Flush to parquet every {FLUSH_EVERY} feeds\n")

    # ── Build feed dicts for all pending indices ──
    feed_tasks = []
    for idx in needs_enrichment:
        row = df.iloc[idx]
        sid = safe_text(row["source_id"])
        feed_tasks.append({
            "idx": idx,
            "source_id": sid,
            "feed": {
                "source_id": sid,
                "source_title": safe_text(row["source_title"]),
                "feed_title": safe_text(row["feed_title"]),
                "feed_description": safe_text(row["feed_description"]),
                "site_url": safe_text(row["site_url"]),
                "article_samples": article_samples.get(sid, []),
            },
        })

    # ── Parallel execution ──
    processed = 0
    failed = 0
    completed = 0
    start_time = time.time()

    with ThreadPoolExecutor(max_workers=workers) as executor:
        # Submit all tasks
        future_map = {}
        for task in feed_tasks:
            future = executor.submit(call_deepseek, task["feed"], api_key)
            future_map[future] = task

        # Collect results as they complete
        for future in as_completed(future_map):
            task = future_map[future]
            idx = task["idx"]
            sid = task["source_id"]
            feed = task["feed"]

            try:
                result = future.result(timeout=10)
            except Exception as e:
                result = None

            title_preview = (feed["feed_title"] or feed["source_title"])[:50]

            if result is None:
                failed += 1
                safe_print(f"  [{completed+1}/{total_needed}] {title_preview}... FAILED")
            else:
                desc = result.get("ai_description", "")
                tags = result.get("ai_tags", "")
                progress[sid] = {"ai_description": desc, "ai_tags": tags}
                df.at[idx, "ai_description"] = desc
                df.at[idx, "ai_tags"] = tags

                if desc.strip():
                    processed += 1

                desc_short = desc[:70] + "…" if len(desc) > 70 else desc
                safe_print(f"  [{completed+1}/{total_needed}] {title_preview}... ✓ {desc_short}")

            completed += 1

            # Periodic flush
            if completed % FLUSH_EVERY == 0:
                elapsed = time.time() - start_time
                rate = processed / elapsed if elapsed > 0 else 0
                remaining = total_needed - completed
                eta_min = remaining / rate / 60 if rate > 0 else 0
                safe_print(f"  --- flush at {completed}/{total_needed} ({rate:.1f}/s, ETA {eta_min:.0f}min) ---", flush=True)
                write_parquet(df)
                # Reset progress for next batch
                progress.clear()
                save_progress(progress)
                processed = 0
                failed = 0
                start_time = time.time()

    # ── Final flush ──
    elapsed = time.time() - start_time
    print(f"\n{'=' * 60}")
    print(f"Batch enriched: {processed} | Failed: {failed}")
    print(f"Time: {elapsed:.0f}s ({processed/elapsed:.1f} feeds/s)" if elapsed > 0 else "")

    if processed > 0:
        write_parquet(df)
        print(f"  ✓ final parquet written")
        progress.clear()
        save_progress(progress)
    else:
        save_progress(progress)
        print(f"  Nothing new to flush")


# ---------------------------------------------------------------------------
# CLI
# ---------------------------------------------------------------------------

def main():
    import argparse
    global PARQUET_PATH, BACKUP_PATH, PROGRESS_PATH, CONTENT_PARTS_DIR

    parser = argparse.ArgumentParser(
        description="Parallel feed enrichment via DeepSeek V4 Flash"
    )
    parser.add_argument("--limit", type=int, default=0)
    parser.add_argument("--reset", action="store_true")
    parser.add_argument("--dry-run", action="store_true")
    parser.add_argument("--workers", type=int, default=DEFAULT_WORKERS,
                        help=f"Concurrent API workers (default: {DEFAULT_WORKERS})")
    parser.add_argument("--parquet", type=Path, default=PARQUET_PATH)
    parser.add_argument("--backup", type=Path)
    parser.add_argument("--progress", type=Path)
    parser.add_argument("--parts-dir", type=Path)
    args = parser.parse_args()

    PARQUET_PATH = args.parquet.resolve()
    BACKUP_PATH = (
        args.backup.resolve()
        if args.backup
        else PARQUET_PATH.with_name(f"{PARQUET_PATH.stem}.backup.parquet")
    )
    PROGRESS_PATH = (
        args.progress.resolve()
        if args.progress
        else PARQUET_PATH.with_name(f"{PARQUET_PATH.stem}.enrich-progress.json")
    )
    CONTENT_PARTS_DIR = args.parts_dir.resolve() if args.parts_dir else None

    print("=" * 60)
    print("FeedMine — Parallel Feed Enricher (DeepSeek V4 Flash)")
    print(f"  Started: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}")
    print(f"  Model: {DEEPSEEK_MODEL}  |  Workers: {args.workers}")
    print("=" * 60)
    print()

    enrich_parquet_parallel(
        limit=args.limit,
        reset=args.reset,
        dry_run=args.dry_run,
        parts_dir=CONTENT_PARTS_DIR,
        workers=args.workers,
    )


if __name__ == "__main__":
    main()
